...
{
    int index = combination_idx;
    assert(index < m_scores.size());
    const auto& itr = m_scores[index].find(read_id);
    assert(itr != m_scores[index].end());
    return itr->second;
}
...
void VariantGroup::empty_variants(){
    m_variants.clear();
}
void VariantGroup::add_variants(Variant& v){
    m_variants.push_back(v);
}
bool VariantGroup::isEmpty(){
    return m_variants.empty();
}
std::vector<Variant> VariantGroup::get_all_variants(){
    return m_variants;
}